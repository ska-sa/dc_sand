 ...
 *          ibverbs library.
 * 
 * This program uses the ibverbs API to transmit raw UDP ethernet data out on a Mellanox NIC. This implementation both 
 * bypasses the kernel and is CPU light.  Once a packet is ready to transmit, it is DMAd directly from a userpace buffer 
 * to the NIC without CPU intervention. With this approach, very high transmit speeds can be achieved (speeds >90 Gbps 
 * have been observed). 
 * 
 * The downside to using losing the kernel stack is that the user has to generate all the packet headers 
 * manually. This program demonstrates how to do this.
 * 
...
/** 
 *  Function that fills all the network headers for a UDP datagram.
 * 
 *  This function generates a raw ethernet packet including the udp, ipv4 and ethernet headers. Ideally the ip addresses
 *  and port numbers should be passed as parameters but at the moment they are just read from the SOURCE_IP_ADDRESS, 
 *  DESTINATION_IP_ADDRESS and UDP_PORT macros defined at the top of the file. Additionally the ethernet mac addresses
 *  need to be populated in the ethernet header. There is probably a way to do this automatically, however for this demo
 *  it was easiest to just read the linux arp table(I used the arp package) and create the SOURCE_MAC_ADDRESS and 
 *  DESTINATION_MAC_ADDRESS macros manually.
 *  
 *  The function also generates the relativly lightweight IP checksum. The UDP header and ethernet checksums are not 
 *  calculated. The ethernet checksum will get generated by the NIC while the UDP checksum is expensive and not checked 
 *  on MeerKATs systems.
 * 
 *  \param[out] p_network_packet    Pointer to network packet to use.
 */
void populate_packet(struct network_packet * p_network_packet);
...
int main()
{
    ...
    /* 11. Cleanup. As in ibverbs_rx.c, there should be cleanup functions here releasing all memory and ibv resources.
     * I have not doen it because I am lazy - be a better than me.
     */
